...
> BlockIdentifierPage get_block_identifiers(platform, network)
...
with ubiquity.ubiquity_openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Block Identifiers
        api_response = api_instance.get_block_identifiers(platform, network)
        pprint(api_response)
    except ubiquity.ubiquity_openapi_client.ApiException as e:
...
[**BlockIdentifierPage**](BlockIdentifierPage.md)
